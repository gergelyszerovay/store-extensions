...
>(options: {
  ...
  service: (
    store: SignalStoreSlices<Input['state']> &
      ...
      StateSignal<{}>,
    rxParams: RxParams,
  ) => Observable<Array<Partial<Input['state'] & {}> | PartialStateUpdater<Input['state']>>>;
      ...
      return {
        [fetchKey]: rxMethod<RxParams>(
          pipe(
            ...
            switchMap((rxParams) =>
              // @ts-ignore
              runInInjectionContext(injector, () => service(store, rxParams)),
            ),
            tapResponse(
              (response) => {
...
function setFetching<ActionName extends string>(
  ...
  return {
    [`${actionName}RequestState`]: HttpRequestStates.FETCHING,
  } as WithDataServiceSlice<ActionName>;
}
...
function setFetched<ActionName extends string>(
  ...
  return {
    [`${actionName}RequestState`]: HttpRequestStates.FETCHED,
  } as WithDataServiceSlice<ActionName>;
}
 ...
 */
export function extractHttpErrorMessage(error: DeepReadonly<HttpErrorResponse>): HttpRequestError {
